...
enum CSVExporter {
    ...
    static func makeCSV(
        logs: [TimeLog], clipTo range: ClosedRange<Date>? = nil,
        ...
        var rows = [L10n.string("csv.timelogs.header", locale: locale)]
        for log in sorted {
            ...
            let name = log.project?.name ?? L10n.string("（削除済み）", locale: locale)
    ...
    /// 作業内容別の集計 CSV を生成する。
    static func makeNoteSummaryCSV(totals: [NoteTotal], locale: Locale = .current) -> String {
        var rows = [L10n.string("csv.notes.header", locale: locale)]
        for total in totals {
            let fields = [
                escape(total.note ?? L10n.string("未分類", locale: locale)),
    ...
    static func makeDailyWorkCSV(
        summaries: [DailyWorkSummary], calendar: Calendar = .current,
        ...
        var rows = [L10n.string("csv.daily.header", locale: locale)]
